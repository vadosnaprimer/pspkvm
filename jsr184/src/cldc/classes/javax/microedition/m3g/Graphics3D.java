...
public final class Graphics3D {
	...
	if (target instanceof Graphics)
	{
	    ...
	    //System.out.println("Graphics3D: bindTarget");
    ...
    {
	//System.out.println("Graphics3D: releaseTarget");
	if (this.currentGLCanvas != null)
	{
		...
		//System.out.println("Graphics3D: swapBuffers");
		GLContext ctx = this.currentGLCanvas.getContext();
		ctx.swapBuffers();
		ctx.detach(); //detach the MIDP graphics context;
	    }
	}
	...
	//System.out.println("Graphics3D: setViewport " + gl);
	...
	if(node instanceof Mesh) {
	    ...
	    //System.out.println("Graphics3D - render :" + subMeshes + " submeshes");
	...
	pos.position(0);
	gl.glVertexPointer(positions.getComponentCount(), GL.GL_BYTE, 0, pos);
	...
	if(normals != null) {
	    ...
	    gl.glNormalPointer(GL.GL_BYTE, 0, norm);
	...
	if(colors != null) {
	    ByteBuffer buffer = (ByteBuffer)colors.getBuffer();
	    buffer.position(0);
	...
	for(int i = 0; i < 8; ++i) {
	    ...
	    if(texcoords != null) {
		ByteBuffer tex = (ByteBuffer)texcoords.getBuffer();
		tex.position(0);
	...
	//System.out.println("[Graphics3D] render Triangles ["+triangles.getIndexCount()+"]");
	if(triangles instanceof TriangleStripArray)
	{
	    ByteBuffer indices = triangles.getBuffer();
	    //print the index buffer
	    indices.position(0);
	    gl.glDrawElements(GL.GL_TRIANGLE_STRIP, triangles.getIndexCount(), GL.GL_UNSIGNED_BYTE, indices);
	}
	else{
	    gl.glDrawElements(GL.GL_TRIANGLES, triangles.getIndexCount(), GL.GL_UNSIGNED_BYTE, triangles.getBuffer());
	}
	gl.glPopMatrix();
    }
    ...
    {
	//System.out.println("Graphics3D: render");
	...
	setCamera(c, t);
	//+++++++++++++++++test,set perspective viewport
	//gl.glMatrixMode(GL.GL_PROJECTION);
	//gl.glLoadIdentity();
	//perspective(90.f, (float)viewportWidth/viewportHeight, 0.1f, 100.f);
	gl.glFinish();
	//++++++++++++++++++++++++++++++++++++++++++++++
	// Setup lights
	resetLights();
